...
app.post('/api/game-rooms/:roomId/refresh-prices', async (req, res) => {
  try {
    ...
    const positions = await dbAll(`SELECT symbol, avg_cost FROM GameRoomPosition WHERE room_id = ?`, [roomId]);
    ...
    try {
      for (const p of positions) {
        ...
        await dbRun(
          `UPDATE GameRoomPosition SET last_price = ?, updated_at = strftime('%s','now') WHERE room_id = ? AND symbol = ?`,
    ...
    // After refreshing prices we may want to update participant equity/cash snapshots or record history externally (frontend calls /record-history)
    return res.json({ updated: results });
  } catch (err) {
